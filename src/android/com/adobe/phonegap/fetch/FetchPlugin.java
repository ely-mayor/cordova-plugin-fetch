...
public class FetchPlugin extends CordovaPlugin {
    ...
    public boolean execute(final String action, final JSONArray data, final CallbackContext callbackContext) {
        if (action.equals("fetch")) {
            cordova.getThreadPool().execute(() -> fetchOperation(data, callbackContext));
            return true;
        } else if (action.equals("setTimeout")) {
            this.setTimeout(data.optLong(0, DEFAULT_TIMEOUT));
            return true;
    ...
    private void fetchOperation(JSONArray data, CallbackContext callbackContext) {
        try {
            ...
            if (!postBody.equals("null")) {
                requestBuilder.method(method, RequestBody.create(MEDIA_TYPE_JSON, postBody));
            } else {
                requestBuilder.method(method, null);
            }
            ...
            Log.e(LOG_TAG, "Error executing fetch operation: " + e.getMessage());
            callbackContext.error(e.getMessage());
        }
    }
    ...
    private void handleResponse(Response response, CallbackContext callbackContext) throws IOException {
        ...
        try {
            ...
            Log.e(LOG_TAG, "Error handling response: " + e.getMessage());
        }
